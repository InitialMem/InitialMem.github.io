...
我们无法直接在浏览器里去插入图片，文字等元素，所以需要一种方式来告诉浏览器怎么显示它们，HTML就是一种标记语言，用来描述网页

## 什么是web标准

为了让不同的浏览器对同一网页显示效果相同，所以我们需要制定一套标准，这个web标准就是w3c组织制定的，包括**结构、表现**、**行为**这三方面
...
| 复制当前行         | shift + alt + 下箭头                           |
| 选择多个相同的单词 | ctrl + d                                       |
| 添加多个光标       | shift + alt + 点击或者上下箭头                 |
| 快速跳行           | ctrl + g                                       |
| 选择区域           | shift + alt + 拖动鼠标                         |
| 放大缩小界面       | ctrl + 加号或者减号                            |
| 自定义快捷键       | 块注释(ctrl + shift + /) 原来是shift + alt + a |
